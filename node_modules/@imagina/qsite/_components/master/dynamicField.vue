<template>
  <div id="dynamicFieldComponent" class="relative-position" v-if="success">
    ...
    <!--Input-->
    <q-input :filled="filled" v-model="responseValue" @keyup.enter="$emit('enter')" v-if="loadField('input')" :label="fieldLabel"
             v-bind="{ bgColor:(filled ? '' : 'white'), outlined : (filled ? false : true), dense : (filled ? false : true), ...(field.props || {}) }"/>
    <!--Date-->
    <q-input
      :filled="filled"
    ...
    <!--Select-->
    <q-select :filled="filled" v-model="responseValue" :options="formatOptions" :label="fieldLabel"
              ...
              v-if="loadField('select')" :loading="loading" use-input v-bind="{
                emitValue : true, mapOptions : true, outlined:true, dense : (filled ? false : true),
                bgColor: (filled ? '' : 'white'), style:'width: 100%', ...(field.props || {})}">
      <!--No options slot-->
    ...
    <!--multiSelect-->
    <q-field :filled="filled" v-model="responseValue" v-if="loadField('multiSelect')" label=""
             v-bind="{borderless : true, dense : true, ...(field.props || {})}">
      <recursive-select v-model="responseValue" class="bg-white full-width" :items="options"/>
    </q-field>
    <!--Checkbox-->
    <q-field :filled="filled" v-model="responseValue" v-if="loadField('checkbox')" label="" class="checkbox-field"
             v-bind="{borderless : true, dense : true, ...(field.props || {})}">
      <q-checkbox v-model="responseValue" :label="fieldLabel" v-bind="field.props || {}"/>
    </q-field>
    <!--Media-->
    <q-field :filled="filled" v-model="responseValue" v-if="loadField('media')" label=""
             v-bind="{borderless : true, dense : true, ...(field.props || {})}">
